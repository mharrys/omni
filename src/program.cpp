...
Program::Program(std::string vertex_shader_path, std::string fragment_shader_path)
    : gl(new GLProgram()),
      vert(ShaderType::VERTEX, vertex_shader_path),
      frag(ShaderType::FRAGMENT, fragment_shader_path)
{
    gl->program = 0;
    load();
}
